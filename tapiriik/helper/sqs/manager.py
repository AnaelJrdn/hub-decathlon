...
class SqsManager:
    ...
    _QUEUE_NAME = AWS_SQS_QUEUE_NAME
    _resource = None
    _queue = None
    _queue_url = None
    _messages = []
    ...
    # Function use to get a specific queue from AWS SQS, it uses self._QUEUE_NAME as parameter
    # Queue will be available in self._queue
    def get_queue(self, queue_name=None):

        if queue_name:
            self._QUEUE_NAME = queue_name
        print("[Helper SQS]--- Get queue %s in %s resource" % (self._QUEUE_NAME, self._AWS_REGION))
